...
public class SwipeUI : MonoBehaviour
{
    ...
    [field: SerializeField] public List<GameObject> circleContents = new List<GameObject>();             // 현재 페이지를 나타내는 원 Image UI들의 Transform
    [SerializeField]
    ...
    [SerializeField] public bool selectMenuGenerateComplite = false;
    ...
    private void UpdateSwipe()
    {
        ...
        if (Mathf.Abs(startTouchX - endTouchX) < swipeDistance)
        {
			...
			//print(startTouchX + " " + endTouchX + " " + (startTouchX - endTouchX));
    ...
    private void UpdateCircleContent()
    {
        ...
        for (int i = 0; i < scrollPageValues.Length; ++i)
        {
            ...
            circleContents[i].gameObject.SetActive(false);
            ...
            if (scrollBar.value < scrollPageValues[i] + (valueDistance / 2) && scrollBar.value > scrollPageValues[i] - (valueDistance / 2))
            {
                ...
                circleContents[i].gameObject.SetActive(true);
            }
        }
    }
}
